...
'''
on_start:
  wait_for(nav2_servers, slam_toolbox_active, /map available)
  state = MAPPING_EXPLORE

MAPPING_EXPLORE:
  start_explorer_or_publish_pattern()
  while not done:
    map = latest /map
    coverage = known_cells(map) / total_cells(map)
    if coverage > COVERAGE_THRESH or time > TIMEOUT:
      stop_explorer()
      state = SAVE_MAP
...
SAVE_MAP:
  ts = now()
  out = save_dir + "/map-" + ts
  call_service("/map_saver/save_map", { map_url: out, map_topic: "/map", image_format: "pgm" })
  assert files exist (out.yaml, out.pgm)
  saved_map = out + ".yaml"
  state = SWITCH_TO_LOCALIZE

SWITCH_TO_LOCALIZE:
  lifecycle_deactivate("slam_toolbox")
  set_param("map_server", "yaml_filename", saved_map)
  lifecycle_configure_activate("map_server")
  lifecycle_configure_activate("amcl")
  seed_initialpose_from_tf()   # read map->base_link, publish /initialpose once
  state = NAVIGATE

NAVIGATE:
  # normal AMCL navigation; run TSP/mission if desired
  run_tsp_multi_goal_mission()
'''